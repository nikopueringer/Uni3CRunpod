...
def parse_args_to_cfg():
    ...
    fps = round(iio.immeta(video_path, plugin="pyav")["fps"])
...
def load_data_dict(cfg):
    paths = cfg.paths
    length, width, height = get_video_lwh(cfg.video_path)
    fps = iio.immeta(cfg.video_path, plugin="pyav")["fps"]
    if cfg.static_cam:
...
def render_incam(cfg, colors, save_path):
    incam_video_path = Path(save_path)
    ...
    smplx_coco = make_smplx("supermotion_coco17").cuda()
    # smplx2smpl = torch.load("hmr4d/utils/body_model/smplx2smpl_sparse.pt").cuda()
    faces_smpl = smplx.faces
    # coco17 -> for Uni3C
    kp3d = smplx_coco(**to_cuda(pred['smpl_params_global'])).cpu().float().numpy()
    np.save(cfg.paths.coco17, kp3d)

    # smpl
    smplx_out = smplx(**to_cuda(pred["smpl_params_incam"]))
    pred_c_verts = smplx_out.vertices
    #  for Uni3C
    smplx_global_out = smplx(**to_cuda(pred["smpl_params_global"]))
    np.save(cfg.paths.smplx_vertices, smplx_global_out.vertices.cpu().float().numpy())
    ...
    for i, img_raw in tqdm(enumerate(reader), total=get_video_lwh(video_path)[0], desc=f"Rendering Incam"):
        img = renderer.render_mesh(verts_incam[i].cuda(), None, colors)
        writer.write_frame(img)
    writer.close()
    reader.close()
...
def render_global(cfg):
    ...
    pred = torch.load(cfg.paths.hmr4d_results, weights_only=False)
    smplx = make_smplx("supermotion").cuda()
    smplx2smpl = torch.load("hmr4d/utils/body_model/smplx2smpl_sparse.pt", weights_only=False).cuda()
    faces_smpl = make_smplx("smpl").faces
    J_regressor = torch.load("hmr4d/utils/body_model/smpl_neutral_J_regressor.pt", weights_only=False).cuda()
...
if __name__ == "__main__":
    ...
    smplx_cs_colors = torch.load("hmr4d/utils/body_model/smplx_color.pt", weights_only=False)
    smplx_cs_colors = torch.from_numpy(smplx_cs_colors) / 255.0
    smplx_norm_colors = torch.load("hmr4d/utils/body_model/smplx_canonical_norm.pt", weights_only=False)
    smplx_norm_colors = torch.from_numpy(smplx_norm_colors) / 255.0
    ...
    # ===== Render ===== #
    render_incam(cfg, smplx_cs_colors, cfg.paths.cs_map_video)
    render_incam(cfg, smplx_norm_colors, cfg.paths.canonical_norm_video)