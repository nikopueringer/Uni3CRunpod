...
class Renderer:
    ...
    def render_mesh(self, vertices, background=None, colors=[0.8, 0.8, 0.8], VI=50):
        ...
        if background is None:
            background = (
                np.zeros((self.height, self.width, 3)).astype(np.uint8)
            )
        vertices = vertices.unsqueeze(0)

        if isinstance(colors, torch.Tensor):
            # per-vertex color
            verts_features = colors.unsqueeze(0).to(device=vertices.device, dtype=vertices.dtype)
        else:
            if colors[0] > 1:
        ...
        results = torch.flip(self.renderer(mesh, materials=materials, cameras=self.cameras, lights=self.create_lights()), [1, 2])
